...
class Post:
    ...
    def count_posts_by_category(cls, user_id, category_id):
        get_count = 'select count(1) count from tb_post where visible = 1 and deleted = 0 and user_id = %s and category_id = %s'
    ...
    def list_by_category(cls, user_id, category_id, offset=0, size=10):
        query_list = """select id, title, content, category_id, created, updated, if(updated is NULL, created, updated) last_modified 
                        from tb_post where visible = 1 and deleted = 0 and user_id = %s and category_id = %s order by last_modified desc limit %s, %s"""
        posts = db.query(query_list, user_id, category_id, offset, size)
        return posts
